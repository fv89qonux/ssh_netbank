...
<body>
    <div align="center">
        <form action="/ssh_bank/admin/search" method="post">
                    ...
                    <td height="20px" valign="middle">
                        <div id="noWrap" align="center">
                            <s:if test="account.status.name=='启用'">
                                <input type="button" value="冻结" onclick="javascript:location.href='/ssh_bank/admin/locking?id=${account.accountid} & status.id=${status.id}'">
                            </s:if>
                            <s:else>
                                <input type="button" value="启用" onclick="javascript:location.href='/ssh_bank/admin/enabled?id=${account.accountid} & status.id=1'">
                            </s:else>
                        </div>
                    </td>
                    <td height="20px" valign="center">
                        <div id="noWrap" align="center">
                            <a href="/ssh_bank/admin/del?id=${account.accountid} & status.id=${status.id}"> 删除 </a>
                        </div>
                    </td>
                </tr>
            </s:iterator>
